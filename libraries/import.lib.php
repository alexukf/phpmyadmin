            ...
            if (!empty($import_run_buffer['sql']) && trim($import_run_buffer['sql']) != '') {
                ...
                if (!$cfg['AllowUserDropDatabase']
                    ...
                    if ($run_query
                        ...
                        if ($result != false) {
//echo "<Br />PMA_importRunQuery1: ".$GLOBALS['db'];
                            list($db, $reload) = PMA_lookForUse($import_run_buffer['sql'], $db, $reload);
//echo "<Br />PMA_importRunQuery2: ".$GLOBALS['db'];
                        }
...
{
//echo "<Br />PMA_lookForUse1: globlas db: ".$GLOBALS['db']."XXX<br />";
    if (preg_match('@^[\s]*USE[[:space:]]+([\S]+)@i', $buffer, $match)) {
        $db = trim($match[1]);
        $db = trim($db, ';'); // for example, USE abc;
        
        if (strcmp('`'.$_GLOBALS.'`',$db)) {
...
//echo "<Br /><Br /><Br />PMA_lookForUse2: globlas db: ".$GLOBALS['db']."|db: ".$db."|bffer: ".$buffer."match: ";
//echo "<pre>";
//print_r($match);
//echo "<Br /><Br /><Br /></pre>";
...
//echo "<Br />Ximportlib1: ".$GLOBALS[db];
//echo "<pre>";
//print_r($additional_sql);
//echo "</pre>";
            PMA_importRunQuery($additional_sql[$i], $additional_sql[$i]);
//echo "<Br />importlib2: ".$GLOBALS[db];
        }
    }
